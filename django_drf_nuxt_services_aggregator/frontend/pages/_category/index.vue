<template>
  <div>
    ...
    <Breadcrumbs :current_page="category_name.name" />

    <!-- Сам  список компаний components/pages/Companies-->
    <PagesCompaniesInCategories
      :companies="resp_data"
      :name_slug="
        category_name.declension_one_p !== null
          ? category_name.declension_one_p
          : category_name.name
      "
...
export default {
  ...
  async asyncData({ $axios, store, req, route }) {
    ...
    if (Object.keys(route.query).length > 1) {
      ...
      params = `page=${page}&subdomain=${subdomain}&subcategory=${route.params.category}`;
    }
  ...
  head() {
    return {
      title: `Ремонт ${this.category_name.declension_one_p} в ${this.$store.getters["cities/activeCity"].declension_p} - ${this.resp_data.count} адреса, цены, отзывы`,
      meta: [
        {
          hid: "description",
  ...
  methods: {
    ...
    async pageData() {
      ...
      if (Object.keys(this.$route.query).length > 1) {
        ...
        url = `api/v1/companies/?page=${this.page_Pagination}&subdomain=${this.subdomain}&subcategory=${this.$route.params.category}`;
        this.$router.push(
          `/${this.$route.params.category}/` + `?page=${this.page_Pagination}`
        );
      }
    ...
    async RemoveFilter() {
      let url = `api/v1/companies/?page=${this.page_Pagination}&subdomain=${this.subdomain}&subcategory=${this.$route.params.category}`;
      this.loader = true;
      this.filter = false;
      this.filter_params = "";
    ...
    async OnFilters(
      ...
      await this.$axios
        .$get(
          "api/v1/companies/?" +
            `page=1&subcategory=${this.$route.params.category}&` +
